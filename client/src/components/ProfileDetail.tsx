...
const ProfileDetail = () => {
    ...
    const myList:Array<string> = ["회원정보 수정", "작성한 큐레이션", "좋하요한 큐레이션", "구독하는 큐레이터"] ;
    const anotherList:Array<string> = ["작성한 큐레이션", "좋하요한 큐레이션"] ;
...
<ProfileDetailContainer>
    ...
    onClick={() => {
        setSelected(idx);}}>{e}</li>
       ))
    }   
    {/* {anotherList.map((e,idx) => (
        <li key={`another ${idx}`}>{e}</li>
    ))} */}
    </ul>
    </ProfileAside>
    <ProfileDetailMain>
